...
function createautocomplete(id,url,hn){
	...
	mydivshow.style.textAlign = "left";
	mydiv.style.border = "1px solid transparent";
	mydivshow.style.backgroundColor = "#fcf8e3";
	mydivshow.style.height = "auto";
	mydivshow.style.width = "auto";
	...
	/*mydivshow.onblur = function(){
		this.style.display = "none";
	};*/
	/*mydivshow.onmouseleave = function(){
		var valuz = this.value;
		var selfid = this.sid;
		var selfurl = this.url;
		document.getElementById(selfid+'_div_show').style.display = "none";
	};*/
	myobj.oninput = function(){
		...
		//document.getElementById(selfid+'_div_show').focus();
		document.getElementById(selfid+'_div_show').innerHTML = "";
		var x = document.getElementById(selfid+'_div_show').getElementsByTagName('*');
		var i;
		for (i = 0; i < x.length; i++) {
		  x[i].classList.add("my_class1");
		}
		if(valuz.length > 2){
					...
					if(myrec.status == 200 || myrec.status == 0) {
						...
						if(data.length){
							...
							data.forEach(function(val) {
								val = ''+val;
								var arr = val.split(",");
								var word = arr[1].toLowerCase().replace(valuz.toLowerCase(),"<span  style='color:brown;'>"+valuz.toLowerCase()+"</span>").toUpperCase();
								//console.log(word);
	...
	myobj.dataSelect = function(oid,val1,val2){
		...
		document.getElementById(oid+'_div_show').innerHTML = "";
		document.getElementById(oid+'_div_show').style.display = "none";
	};
	if(typeof document.onmousedown === 'function'){
		var old = document.onmousedown;
		document.onmousedown = function (e) {
            if (old) {
                old(arguments[0]);
            }
			if(old.toString().indexOf('my_class1') == -1){
				var target = e.target; // jQuery wrapped target
				var classes = target.getAttribute('class');
				//console.log(classes);
				if(classes==null || classes.toLowerCase().indexOf("my_class1")==-1){
